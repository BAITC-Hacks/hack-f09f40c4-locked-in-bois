...
3. **ИИ-аналитик:** сильные стороны, риски, последствия, компромиссы и рекомендация. Кнопка «Применить рекомендацию» загружает рекомендованный план обратно в кабинет.
   Чтобы увидеть агента: задайте `LLM_PROVIDER=openai` и `OPENAI_API_KEYS` в `.env`, перезапустите сервер; в блоке «Аналитик» раскройте «Вызовы инструментов». Без ключа — шаблонный анализ.
   ![Вызовы инструментов ИИ](docs/screenshots/agent.png)
